...
static CVI_S32 CVI_Test_SetConfig(CVI_CIPHER_HANDLE chnHandle, CVI_UNF_CIPHER_ALG_E alg,
				  ...
				  const CVI_U8 *u8KeyBuf, const CVI_U8 *u8IVBuf)
{
...
CVI_S32 CCM_AES128(void)
{
	...
	// Set the tag before decryption (after ConfigHandleEx but before Decrypt)
	s32Ret = CVI_UNF_CIPHER_SetTag(hTestchnid, out_tag, sizeof(aes_tag));
	if (s32Ret != CVI_SUCCESS) {
		CVI_ERR_CIPHER("Set tag failed.\n");
		goto __CIPHER_EXIT__;
	}
...
static CVI_S32 GCM_AES128(void)
{
	...
	s32Ret = CVI_UNF_CIPHER_SetTag(hTestchnid, out_tag, 16);
	if (s32Ret != CVI_SUCCESS) {
		CVI_ERR_CIPHER("Set tag failed.\n");
		goto __CIPHER_EXIT__;
	}
