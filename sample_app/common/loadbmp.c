...
CVI_S32 GetBmpInfo(const char *filename, OSD_BITMAPFILEHEADER *pBmpFileHeader, OSD_BITMAPINFO *pBmpInfo)
{
	...
	SAMPLE_PRT("bmp width(%d) height(%d) bpp(%d) compression(%d)\n"
	...
	if (pBmpInfo->bmiHeader.biCompression == 3)
		SAMPLE_PRT("bitmask a(%#x) r(%#x) g(%#x) b(%#x)\n", pBmpInfo->bitfield.a_mask
...
int LoadBMP(const char *filename, OSD_LOGO_T *pVideoLogo, OSD_COLOR_FMT_E enFmt, CVI_BOOL update_logo)
{
	...
	case BI_RGB:
		SAMPLE_PRT("Non-compressed bitmap file!\n");
		break;
	case BI_RLE4:
	case BI_RLE8:
		SAMPLE_PRT("only support non-compressed or bitfile bitmap file!\n");
		return -1;
	case BI_BITFIELDS:
		SAMPLE_PRT("Bitfile bitmap file!\n");

		if ((Bpp != 2) && (Bpp != 4)) {
			SAMPLE_PRT("Compression(3):RGB bitmap bitfiled format bpp(%d) not supported!\n", Bpp);
			return -1;
		}
	...
	if (bmpInfo.bmiHeader.biHeight < 0) {
		SAMPLE_PRT("bmpInfo.bmiHeader.biHeight < 0\n");
		return -1;
	}
	if (bmpInfo.bmiHeader.biHeight < 0) {
		SAMPLE_PRT("bmpInfo.bmiHeader.biHeight < 0\n");
	...
	if (pOrigBMPBuf == NULL) {
		SAMPLE_PRT("not enough memory to malloc!\n");
		fclose(pFile);
		return -1;
	}
	...
	if (h > pVideoLogo->height) {
		SAMPLE_PRT("Bitmap's height(%d) is bigger than canvas's height(%d). Load bitmap error!\n", h,
		       pVideoLogo->height);
		free(pOrigBMPBuf);
		fclose(pFile);
		return -1;
	...
	if (w > pVideoLogo->width) {
		SAMPLE_PRT("Bitmap's width(%d) is bigger than canvas's width(%d). Load bitmap error!\n", w,
		       pVideoLogo->width);
		free(pOrigBMPBuf);
		fclose(pFile);
		return -1;
	...
	if (fread(pOrigBMPBuf, 1, h * stride, pFile) != (h * stride)) {
		SAMPLE_PRT("fread (%d*%d)error!line:%d\n", h, stride, __LINE__);
		perror("fread:");
	}
	...
	for (i = 0; i < h; i++) {
		for (j = 0; j < w; j++) {
			if (Bpp == 3) {
				...
				default:
					SAMPLE_PRT("file(%s), line(%d), no such format!\n", __FILE__, __LINE__);
					break;
				}
			} else if ((Bpp == 2) || (Bpp == 4)) {
				pStart = pOrigBMPBuf + ((h - 1) - i) * stride + j * Bpp;
				...
				if (i == 0 && j == 0) {
					SAMPLE_PRT("Func: %s, line:%d.\n", __func__, __LINE__);
					SAMPLE_PRT("Bpp: %d, bmp stride: %d, Canvas stride: %d, h:%d, w:%d.\n",
				...
				*pDst = OSD_MAKECOLOR_U16_A(a, r, g, b, s_OSDCompInfo[enFmt]);
				...
				SAMPLE_PRT("Error Bpp: %d in Func: %s, line:%d\n",	Bpp, __func__, __LINE__);
		}
	}
...
char *GetExtName(char *filename)
{
	...
	if (filename == NULL) {
		SAMPLE_PRT("filename can't be null!");
		return NULL;
	}
...
int LoadImageEx(const char *filename, OSD_LOGO_T *pVideoLogo, OSD_COLOR_FMT_E enFmt)
{
	...
	if (ext == CVI_NULL) {
		SAMPLE_PRT("LoadImageEx error!\n");
		return -1;
	}

	if (strcmp(ext, "bmp") == 0) {
		if (LoadBMP(filename, pVideoLogo, enFmt, CVI_TRUE) != 0) {
			SAMPLE_PRT("OSD_LoadBMP error!\n");
			return -1;
		}
	} else {
		SAMPLE_PRT("not supported image file!\n");
		return -1;
	}
