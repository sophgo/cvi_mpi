...
function _audio_resample_play_test()
{
	...
	_check_keyworkd_exist2 $tmp_result $pass_keyword "_audio_resample_play_test" $output_result
}
...
function unit_pcm_write_test()
{
	...
	if [ "$?" = "1" ]
	then
		echo "unit_pcm_write_test $pass_keyword" >> $output_result
	else
...
function unit_pcm_read_test()
{
	...
	if [ "$?" = "1" ]
	then
		echo "unit_pcm_read_test $pass_keyword" >> $output_result
	else
...
function unit_transcode_test()
{
	...
	if [ "$?" = "1" ]
	then
		echo "unit_transcode_test encode $pass_keyword" >> $output_result
	else
	...
	if [ "$?" = "1" ]
	then
		echo "unit_transcode_test decode $pass_keyword" >> $output_result
	else
...
function unit_resample_test()
{
	...
	if [ "$?" = "1" ]
	then
		echo "unit_resample_test resample $pass_keyword" >> $output_result
	else
