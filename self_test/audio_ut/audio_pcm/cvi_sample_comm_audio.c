...
static SAMPLE_AI_S   gs_stSampleAi[AUD_AI_DEV_MAX_NUM * AUD_AI_MAX_CHN_NUM];
static SAMPLE_AENC_S gs_stSampleAenc[AUD_AENC_MAX_CHN_NUM];
static SAMPLE_ADEC_S gs_stSampleAdec[AUD_ADEC_MAX_CHN_NUM];
static SAMPLE_AO_S   gs_stSampleAo[AUD_AO_DEV_MAX_NUM];
static SAMPLE_ADEC_AO_S gs_stSampleAdecAo[AUD_ADEC_MAX_CHN_NUM];
...
{
	enSample = enSample;
	printf("This function is not needed\n");
	return CVI_SUCCESS;
}
...
CVI_VOID *SAMPLE_COMM_AUDIO_AdecProc(CVI_VOID *parg)
{
	...
	pu8AudioStream = (CVI_U8 *)malloc(sizeof(CVI_U8) * AUD_MAX_STREAM_LEN);
				       ...
				       AUDIO_DEV AoDev, AO_CHN AoChn)
{
	...
	pstAi = &gs_stSampleAi[AiDev * AUD_AI_MAX_CHN_NUM + AiChn];
...
CVI_S32 SAMPLE_COMM_AUDIO_DestoryAllTrd(CVI_VOID)
{
	...
	for (u32DevId = 0; u32DevId < AUD_AI_DEV_MAX_NUM; u32DevId++) {
		for (u32ChnId = 0; u32ChnId < AUD_AI_MAX_CHN_NUM; u32ChnId++) {
	...
	for (u32ChnId = 0; u32ChnId < AUD_AENC_MAX_CHN_NUM; u32ChnId++) {
	...
	for (u32ChnId = 0; u32ChnId < AUD_ADEC_MAX_CHN_NUM; u32ChnId++) {
	...
	for (u32ChnId = 0; u32ChnId < AUD_AO_DEV_MAX_NUM; u32ChnId++) {
