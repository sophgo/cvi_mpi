...
#define CVI_AUDIO_BLOCK_MODE -1
	...
	while (s32Count--) {
		...
		s32Ret = CVI_AI_ReleaseFrame(s32Dev, s32Chn,
					 &stFrame,
					 &stAecFrm);
		if (s32Ret != CVI_SUCCESS) {
			printf("[Error]CVI_AI_ReleaseFrame none!!\n");
			break;
		}
	}
...
static struct SAMPLE_AI2EXTRES_S   gs_stSampleAiExtRes[AUD_AI_DEV_MAX_NUM *
				       AUD_AI_MAX_CHN_NUM];
		...
		FILE *pResFd)
{
	...
	pstAi2ExtRes = &gs_stSampleAiExtRes[AiDev * AUD_AI_MAX_CHN_NUM + AiChn];
...
CVI_S32 SAMPLE_COMM_AUDIO_DestoryTrdAiExtRes(AUDIO_DEV AiDev, AI_CHN AiChn)
{
	...
	pstAi = &gs_stSampleAiExtRes[AiDev * AUD_AI_MAX_CHN_NUM + AiChn];
	if (pstAi->bStart) {
...
//by testing :/double thread mode 160 failure
	...
	/* bind AI to AO channel */
	s32Ret = SAMPLE_COMM_AUDIO_CreatTrdAiAo(AiDev, AiChn, AoDev, AUD_AO_SYSCHN_CHNID);
	if (s32Ret != CVI_SUCCESS) {
		SAMPLE_DBG(s32Ret);
		goto AIAO_ERR1;
	}
...
CVI_S32 SAMPLE_AUDIO_AiVqeProcessAo(CVI_VOID)
{
	...
	if (u32AiVqeType == 1) {
		...
		printf("Not support Record VQE ... / Only support upstream(talk) VQE\n");
		goto VQE_ERR2;
...
CVI_VOID *SAMPLE_AUDIO_AEC_LOOP_TEST_GET_FRAME(CVI_VOID *parg)
{
	...
	while (s32RecLoops--) {
		...
		s32Ret = CVI_AI_ReleaseFrame(0, 0,
					 &stFrame,
					 &stAecFrm);
	}
...
CVI_S32 SAMPLE_AUDIO_GET_AUDIO_FRAME_BY_FRAME(CVI_VOID)
{
	...
	while (s32RecLoops--) {
		...
		s32Ret = CVI_AI_ReleaseFrame(s32DevId, 0,
					 &stFrame,
					 &stAecFrm);
		if (s32Ret != CVI_SUCCESS) {
			printf("[Error]CVI_AI_ReleaseFrame none!!\n");
...
void *play_audio_file(AIO_ATTR_S *pstAioAttr, FILE *fp)
{
	...
	while (1) {
		...
		if (num_readbytes > 0) {
			...
			//printf("ao send u32len[%d]\n", stFrame.u32Len);
			s32Ret = CVI_AO_QueryChnStat(0, 0, &stAoChnState);
			if (s32Ret != CVI_SUCCESS)
				printf("CVI_AO_QueryChnStat failed with %#x!\n", s32Ret);
