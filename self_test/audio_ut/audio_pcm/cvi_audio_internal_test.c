...
#define MAX_BUFFERING_DEPTH  300  /* #define AUD_MAX_FRAME_NUM		300 */
...
CVI_S32 _cvi_audio_recordfile_sample(CVI_S32 DevId, CVI_S32 rate, CVI_S32 channel, ST_AudioUnitTestCfg *testCfg)
{
	...
	stThreadInfo.card = card;
	ao_attrs.enSamplerate = rate;
	ao_attrs.enBitwidth = AUDIO_BIT_WIDTH_16;
	ao_attrs.enWorkmode = AIO_MODE_I2S_MASTER;
	ao_attrs.enSoundmode = (channel == 2 ? AUDIO_SOUND_MODE_STEREO :
				 AUDIO_SOUND_MODE_MONO);
	ao_attrs.u32EXFlag = 0;
	ao_attrs.u32FrmNum = 10;
	ao_attrs.u32PtNumPerFrm = period_size;
	ao_attrs.u32ChnCnt = 0;
	ao_attrs.enI2sType = AIO_I2STYPE_INNERCODEC;

	err = audio_pcm_open(&stThreadInfo, device, &ao_attrs, AUDIO_IN);
	if (err < 0) {
		ERR_PRINTF("audio_pcm_open fail ret:%d, exit output thread\n", err);
		return CVI_FAILURE;
	}
...
//	_preflush_audin_frm_record(capture_handle, period_size, (period_size * channel * 2), s32sample_rate);
	...
	CVI_S32	size = period_size * 2 * 2;//default channel is stereo
	...
	while (loops > 0) {
		loops--;
		err = audio_pcm_read(&stThreadInfo, buffer, size);
		if (err > 0) {
			/* EPIPE means overrun */
			//ERR_PRINTF("[%s]\n", pcm_get_error(capture_handle));
			ERR_PRINTF("[pcm_read] [%d]\n", size);
		}
...
//sample code
	...
	ao_attrs.enBitwidth = AUDIO_BIT_WIDTH_16;
	ao_attrs.enWorkmode = AIO_MODE_I2S_MASTER;
	ao_attrs.enSoundmode = (channels == 2 ? AUDIO_SOUND_MODE_STEREO :
				 AUDIO_SOUND_MODE_MONO);
	ao_attrs.u32EXFlag = 0;
	ao_attrs.u32FrmNum = 10;
	ao_attrs.u32PtNumPerFrm = frames;
	ao_attrs.u32ChnCnt = 0;
	ao_attrs.enI2sType = AIO_I2STYPE_INNERCODEC;

	err = audio_pcm_open(&stThreadInfo, device, &ao_attrs, AUDIO_OUT);
	if (err < 0) {
		ERR_PRINTF("audio_pcm_open fail ret:%d, exit output thread\n", err);
		return CVI_FAILURE;
	}
	...
	while (1) {
		...
		pcm = audio_pcm_write(&stThreadInfo, buff, buff_size);
		if (pcm > 0)
			ERR_PRINTF("[[pcm_write]][%d]\n", pcm);
	}
...
CVI_S32 cvi_audio_set_dbg_option(ST_AudioUnitTestCfg *testCfg)
{
	...
	if (testCfg->bOptCfg == CVI_TRUE) {
		...
		stThreadInfo.card = s32CardId;
		ao_attrs.enSamplerate = s32Rate;
		ao_attrs.enBitwidth = AUDIO_BIT_WIDTH_16;
		ao_attrs.enWorkmode = AIO_MODE_I2S_MASTER;
		ao_attrs.enSoundmode = (s32Channels == 2 ? AUDIO_SOUND_MODE_STEREO :
						AUDIO_SOUND_MODE_MONO);
		ao_attrs.u32EXFlag = 0;
		ao_attrs.u32FrmNum = 10;
		ao_attrs.u32PtNumPerFrm = s32PeriodSize;
		ao_attrs.u32ChnCnt = 0;
		ao_attrs.enI2sType = AIO_I2STYPE_INNERCODEC;
		...
		if (bRecord == CVI_TRUE) {
			...
			err = audio_pcm_open(&stThreadInfo, s32DevId, &ao_attrs, AUDIO_IN);
			if (err < 0) {
				ERR_PRINTF("audio_pcm_open fail ret:%d, exit output thread\n", err);
				return CVI_FAILURE;
			}

			buffersize = s32PeriodSize * 2 * DEFAULT_BYTES_PER_SAMPLE;//default read channels is stereo
			printf("[buffersize][%d]\n", buffersize);
			...
			while (loops--) {
				err = audio_pcm_read(&stThreadInfo, buffer, buffersize);
				if (err > 0) {
					printf("pcm_read error:[%d][%d]\n", err, buffersize);
				}
			...
			err = audio_pcm_open(&stThreadInfo, s32DevId, &ao_attrs, AUDIO_OUT);
			if (err < 0) {
				ERR_PRINTF("audio_pcm_open fail ret:%d, exit output thread\n", err);
				return CVI_FAILURE;
			}
			...
			while (loops--) {
				...
				err = audio_pcm_write(&stThreadInfo, buffer, buffersize);
				if (err > 0) {
					//pcm_read error
					printf("pcm_write error:[%d][%d]\n", err, buffersize);
				}
			}
			fclose(fp_tmp);
...
CVI_S32 _cvi_audio_playfile_sample32(CVI_CHAR *filename, CVI_S32 DevId)
{
	...
	pcm = audio_pcm_open(&stThreadInfo, device, &ao_attrs, AUDIO_OUT);
	if (pcm < 0) {
		ERR_PRINTF("audio_pcm_open fail ret:%d, exit output thread\n", pcm);
		return CVI_FAILURE;
	}
	...
	while (loops) {
		...
		s32Ret = audio_pcm_write(&stThreadInfo, buff, buff_size);
		if (s32Ret > 0)
			ERR_PRINTF("\n");
...
CVI_S32 _cvi_audio_recordfile_sample2(CVI_S32 DevId, CVI_S32 rate, CVI_S32 channel)
{
	...
	CVI_S32 size = period_size * 2 * 2;//default read channels is stereo
