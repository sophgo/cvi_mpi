...
static CVI_S32 vpss_get_chn_frame_test(CVI_VOID)
{
	...
	CVI_CHAR *pszMD5Sum = "29a45e41ff051d910f53dabd8c65e2c5";

	s32Ret = CVI_VPSS_GetChnFrame(VpssGrp, VpssChn, &stVideoFrame, 1000);
	if (s32Ret != CVI_SUCCESS) {
		UT_PRT("CVI_VPSS_GetChnFrame fail!\n");
		goto exit;
	}
...
exit:
	UT_PRT("client get chn frame %s\n", s32Ret == CVI_SUCCESS ? "pass" : "fail");

	return s32Ret;
}

static CVI_S32 handle_op(CVI_S32 op)
{
...
static CVI_VOID show_help(CVI_VOID)
{
	UT_PRT("%4d: Get Chn Frame\n", VPSS_TEST_CLNT_GET_CHN_FRAME);
}
