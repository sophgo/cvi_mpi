...
CVI_VOID VpssCsc(CVI_U8 *input, struct test_sclr_csc_matrix *cfg, CVI_U8 *output)
{
	CVI_S8 i, j;
	CVI_S32 buf[3] = {0, 0, 0};
	CVI_S32 coeff;
...
{
	VpssCsc(rgbData, &csc_mtrx[SCL_CSC_601_LIMIT_RGB2YUV], yuvData);
}

CVI_VOID VpssCscYuv2Rgb(CVI_U8 yuvData[3], CVI_U8 rgbData[3])
{
	VpssCsc(yuvData, &csc_mtrx[SCL_CSC_601_LIMIT_YUV2RGB], rgbData);
}
