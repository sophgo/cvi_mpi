...
CVI_VOID *enc_proc_send(CVI_VOID *pArgs)
{
	...
	while (1) {
		...
		stFrame.stVFrame.pu8VirAddr[0] = CVI_SYS_Mmap(stFrame.stVFrame.u64PhyAddr[0], ySize + uvSize * 2);
		if (ret != CVI_SUCCESS) {
		...
		if (s32ReadLen == 0) {
			printf("CVI_VENC_SendFrame read end\n");
