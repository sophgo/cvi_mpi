...
#define VPSS_OUTPUT_HEIGHT 480
#define VPSS_OUTPUT_WIDTH 640
...
CVI_S32 vencStartBindVpss(sampleVdec *psvdec)
{
	...
	for (idx = 0; idx < psvdec->u32VdecNumAllChns; idx++) {
		...
		width = VPSS_OUTPUT_WIDTH;
		height = VPSS_OUTPUT_HEIGHT;
...
CVI_S32 SAMPLE_VDEC_START(sampleVdec *psvdec)
{
	...
	for (i = 0; i < psvdec->u32VdecNumAllChns; i++) {
		...
		if (pic->u32BindMode != VDEC_BIND_DISABLE) {
			...
			stVpssGrpAttr.u32MaxW						 = pic->chnInCfg[i].u32BufWidth;
			stVpssGrpAttr.u32MaxH						 = pic->chnInCfg[i].u32BufHeight;
			stVpssGrpAttr.u8VpssDev 					 = 0;

			astVpssChnAttr[VpssChn].u32Width					= VPSS_OUTPUT_WIDTH;
			astVpssChnAttr[VpssChn].u32Height					= VPSS_OUTPUT_HEIGHT;
			...
			s32Ret = SAMPLE_COMM_VPSS_Init(i, abChnEnable, &stVpssGrpAttr, astVpssChnAttr);
			if (s32Ret != CVI_SUCCESS) {
				SAMPLE_PRT("init vpss group failed. s32Ret: 0x%x !\n", s32Ret);
				return s32Ret;
			}

			s32Ret = SAMPLE_COMM_VPSS_Start(i, abChnEnable, &stVpssGrpAttr, astVpssChnAttr);
			if (s32Ret != CVI_SUCCESS) {
		...
		vdecChnInputCfg *pvdcic)
{
	...
	if (enType == PT_JPEG || enType == PT_MJPEG) {
		u32MaxWidth = pvdcic->u32BufWidth?pvdcic->u32BufWidth:JPEGD_MAX_WIDTH;
		u32MaxHeight = pvdcic->u32BufHeight?pvdcic->u32BufHeight:JPEGD_MAX_HEIGHT;
	} else if (enType == PT_H264) {
		u32MaxWidth = pvdcic->u32BufWidth?pvdcic->u32BufWidth:VEDU_H264D_MAX_WIDTH;
		u32MaxHeight = pvdcic->u32BufHeight?pvdcic->u32BufHeight:VEDU_H264D_MAX_HEIGHT;
	} else if (enType == PT_H265) {
		u32MaxWidth = pvdcic->u32BufWidth?pvdcic->u32BufWidth:VEDU_H265D_MAX_WIDTH;
		u32MaxHeight = pvdcic->u32BufHeight?pvdcic->u32BufHeight:VEDU_H265D_MAX_HEIGHT;
	}
		...
		CVI_S32 s32MilliSec)
{
	...
	pvtp->s32MinBufSize = (psvdattr->u32Width * psvdattr->u32Height) >> 1;
...
CVI_S32 parseDecArgv(vdecInputCfg *pic, CVI_S32 argc, char **argv)
{
	...
	while ((ch = getopt_long(argc, argv, "c:w:h:i:o:d:", long_options, &idx)) != -1) {
		...
		case 'c':
...
#if defined(__CV180X__)
			if (!strcmp(pvdcic->codec, "265") || !strcmp(pvdcic->codec, "264"))
				CVI_VDEC_ERR("CV180x don't support 264/265 dec\n");
#elif defined(__CV181X__)
			if (!strcmp(pvdcic->codec, "265"))
				CVI_VDEC_ERR("CV181x don't support 265 dec\n");
			...
			break;
		case 'w':
			pvdcic->u32BufWidth = atoi(optarg);
			CVI_VDEC_TRACE("width = %u\n", pvdcic->u32BufWidth);
			break;
		case 'h':
			pvdcic->u32BufHeight = atoi(optarg);
			CVI_VDEC_TRACE("height = %u\n", pvdcic->u32BufHeight);
			break;
		case 'i':
