...
typedef struct tagUVC_DEVICE_CTX_S {
	...
	uint32_t contrast_val;
	uint32_t hue_val;
	uint32_t saturation_val;
	uint32_t sharpness_val;
	uint32_t power_line_freq_val;
...
#define MAX_BITSTREAM_BUFFER_SIZE 1.5 * 1024 * 1024
#define MJPG_PAYLOAD_SIZE 1024
#define H264_PAYLOAD_SIZE 1024
#define H265_PAYLOAD_SIZE 1024
#define H264_BITRATE 2048
...
static int uvc_video_set_format(UVC_DEVICE_CTX_S *dev)
{
	...
	else if (dev->fcc == V4L2_PIX_FMT_MJPEG || dev->fcc == V4L2_PIX_FMT_H264 || dev->fcc == V4L2_PIX_FMT_HEVC)
...
static int uvc_video_process(UVC_DEVICE_CTX_S *dev)
{
	 ...
	 * streaming yet.
	 */
	// printf("=====uvc_video_process: dev->io=%d,dev->is_streaming=%d\n", dev->io, dev->is_streaming);
	if (!dev->is_streaming)
		return 0;
...
static int uvc_video_reqbufs_userptr(UVC_DEVICE_CTX_S *dev, int nbufs)
{
	...
	if (ret < 0)
	{
		if (ret == -EINVAL)
			...
			printf("UVC: VIDIOC_REQBUFS11 error %s (%d).\n", strerror(errno), errno);
		goto err;
	}
	...
	if (dev->run_standalone)
	{
		...
		for (i = 0; i < rb.count; ++i)
		{
			...
			if (V4L2_PIX_FMT_MJPEG == dev->fcc
				|| V4L2_PIX_FMT_H264 == dev->fcc
				|| V4L2_PIX_FMT_HEVC == dev->fcc)
		...
		case 2:
			switch (cs)
			{
				...
				case UVC_PU_BRIGHTNESS_CONTROL:
					...
					if (CVI_VPSS_SetGrpProcAmp(0, PROC_AMP_BRIGHTNESS, dev->brightness_val) != CVI_SUCCESS) {
				...
				case UVC_PU_CONTRAST_CONTROL:
					...
					if (CVI_VPSS_SetGrpProcAmp(0, PROC_AMP_CONTRAST, dev->contrast_val) != CVI_SUCCESS) {
						CVI_TRACE_LOG(CVI_DBG_ERR, "CVI_VPSS_SetGrpProcAmp NG on grp0!\n");
						return CVI_FAILURE;
					}
					break;
				...
				case UVC_PU_HUE_CONTROL:
					...
					if (CVI_VPSS_SetGrpProcAmp(0, PROC_AMP_HUE, dev->hue_val) != CVI_SUCCESS) {
						CVI_TRACE_LOG(CVI_DBG_ERR, "CVI_VPSS_SetGrpProcAmp NG on grp0!\n");
						return CVI_FAILURE;
					}
					break;
				case UVC_PU_SATURATION_CONTROL:
					...
					if (CVI_VPSS_SetGrpProcAmp(0, PROC_AMP_SATURATION, dev->saturation_val) != CVI_SUCCESS) {
						CVI_TRACE_LOG(CVI_DBG_ERR, "CVI_VPSS_SetGrpProcAmp NG on grp0!\n");
						return CVI_FAILURE;
					}
				case UVC_PU_SHARPNESS_CONTROL:
...
static int uvc_events_process_data(UVC_DEVICE_CTX_S *dev, struct uvc_request_data *data)
{
	...
	switch (dev->control)
	{
		...
		default:
			...
			{
				ret = uvc_events_process_control_data(dev, dev->cur_cs, 2, data);
				if (ret < 0)
					goto err;
	...
	switch (format->fcc)
	{
		...
		case V4L2_PIX_FMT_HEVC:
			if (dev->imgsize == 0)
				printf("WARNING: H265 requested and no image loaded.\n");
			dev->imgsize = H265_PAYLOAD_SIZE;
			target->dwMaxVideoFrameSize = MAX_BITSTREAM_BUFFER_SIZE;
			break;
	}
	target->dwFrameInterval = *interval;
	// printf("===func=%s line=%d dev->control=%u\n", __FUNCTION__, __LINE__,
	// 	   dev->control);
...
int32_t UVC_GADGET_Init(const CVI_UVC_DEVICE_CAP_S *pstDevCaps, u_int32_t u32MaxFrameSize)
{
	...
	{
		s_stUVCDevCtx.imgsize = s_stUVCDevCtx.width * s_stUVCDevCtx.height;
		s_stUVCDevCtx.fcc = V4L2_PIX_FMT_HEVC;
	}
