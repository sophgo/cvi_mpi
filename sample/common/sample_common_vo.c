...
CVI_S32 SAMPLE_COMM_VO_FillIntfAttr(VO_PUB_ATTR_S *pstPubAttr)
{
	...
	case VO_INTF_I80_SW:
		pstPubAttr->sti80Cfg = i80_st7789v_cfg;
		break;
	case VO_INTF_I80_HW:
		//no need, HW-MCU is setup by HWI80_INIT
		break;
	case VO_INTF_CVBS:
	case VO_INTF_YPBPR:
	case VO_INTF_VGA:
...
CVI_S32 SAMPLE_COMM_VO_StopVO(SAMPLE_VO_CONFIG_S *pstVoConfig)
{
	...
	if (s32Ret != CVI_SUCCESS){
		SAMPLE_PRT("SAMPLE_COMM_VO_StopChn fail");
		return CVI_FAILURE;
	}
	s32Ret = SAMPLE_COMM_VO_StopLayer(VoLayer);
	if (s32Ret != CVI_SUCCESS){
		SAMPLE_PRT("SAMPLE_COMM_VO_StopLayer fail");
		return CVI_FAILURE;
	}
	s32Ret = SAMPLE_COMM_VO_StopDev(VoDev);
	if (s32Ret != CVI_SUCCESS){
		SAMPLE_PRT("SAMPLE_COMM_VO_StopDev fail");
		return CVI_FAILURE;
	}
