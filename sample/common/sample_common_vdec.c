...
SendAgainFlag:
				s32Ret = API_COST_TIME_LOG(CVI_VDEC_SendStream(pstVdecThreadParam->s32ChnId, &stStream, -1), "CVI_VDEC_SendStream");
...
SendAgain:
		s32Ret = API_COST_TIME_LOG(CVI_VDEC_SendStream(pstVdecThreadParam->s32ChnId,
				&stStream, pstVdecThreadParam->s32MilliSec), "CVI_VDEC_SendStream");
		if ((s32Ret != CVI_SUCCESS) &&
...
CVI_VOID SAMPLE_COMM_VDEC_CmdCtrl(VDEC_THREAD_PARAM_S *pstVdecSend, pthread_t *pVdecThread)
{
	...
	*pVdecThread = 0;
	...
	while (1) {
		s32Ret = API_COST_TIME_LOG(CVI_VDEC_QueryStatus(pstVdecSend->s32ChnId, &stStatus), "CVI_VDEC_QueryStatus");
		if (s32Ret != CVI_SUCCESS) {
...
CVI_VOID *SAMPLE_COMM_VDEC_GetPic(CVI_VOID *pArgs)
{
	...
	s32Ret = API_COST_TIME_LOG(CVI_VDEC_GetChnAttr(pstVdecThreadParam->s32ChnId, &stAttr), "CVI_VDEC_GetChnAttr");
	if (s32Ret != CVI_SUCCESS) {
...
RETRY_GET_FRAME:
		if (pstVdecThreadParam->eThreadCtrl == THREAD_CTRL_STOP)
			break;
		s32Ret = API_COST_TIME_LOG(CVI_VDEC_GetFrame(
				pstVdecThreadParam->s32ChnId,
		...
		if (s32Ret == CVI_SUCCESS) {
			...
			s32Cnt++;
			s32Ret = API_COST_TIME_LOG(CVI_VDEC_ReleaseFrame(pstVdecThreadParam->s32ChnId, &stVFrame), "CVI_VDEC_ReleaseFrame");
			if (s32Ret != CVI_SUCCESS) {
