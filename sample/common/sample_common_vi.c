...
#define NAME_SIZE 20
	...
	if (CVI_SENSOR_GetSnsTypeByName(value, &Sns_Type) != CVI_SUCCESS) {
		SAMPLE_PRT("ERROR: can not get sensor type by sensor name\n");
	}
	cfg->enSnsType[index] = Sns_Type;
	cfg->enWDRMode[index] = (Sns_Type < SNS_TYPE_LINEAR_BUTT) ?
		WDR_MODE_NONE : WDR_MODE_2To1_LINE;

	if (Sns_Type == SNS_TYPE_WDR_BUTT) {
