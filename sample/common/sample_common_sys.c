...
CVI_BOOL SAMPLE_COMM_FRAME_CompareWithFile(const CVI_CHAR *filename, VIDEO_FRAME_INFO_S *pstVideoFrame)
{
	...
	CVI_U8 *buffer = malloc(stVbCalConfig.u32MainYSize);
	CVI_U32 offset = 0;
...
CVI_S32 SAMPLE_COMM_CheckDDR64MBSize(CVI_BOOL *b64Ddr)
{
	...
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_LOG(CVI_DBG_ERR, "CVI_SYS_GetChipId failed with %#x\n", s32Ret);
		return s32Ret;
	}

	if (u32ChipId == CV1800B || u32ChipId == CV1800C ||
		u32ChipId == CV1820A || u32ChipId == CV1810C)
		*b64Ddr = CVI_TRUE;
	else
		*b64Ddr = CVI_FALSE;
