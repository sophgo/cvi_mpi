...
CVI_S32 SAMPLE_REGION_CreateOverLay(CVI_S32 HandleNum, PIXEL_FORMAT_E pixelFormat)
{
	...
	stRegion.unAttr.stOverlay.bFlip = false;
	stRegion.unAttr.stOverlay.bMirror = false;
		...
		PIXEL_FORMAT_E pixelFormat, CVI_BOOL bCompressed)
{
	...
	if (bCompressed) {
		...
		stBitmap.pData = malloc(stCanvasInfo.stSize.u32Width * stCanvasInfo.stSize.u32Height * 4);
