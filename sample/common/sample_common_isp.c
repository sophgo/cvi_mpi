...
SNS_TYPE_E g_enSnsType[VI_MAX_DEV_NUM] = {
...
CVI_S32 SAMPLE_COMM_ISP_PatchSnsObj(CVI_U32 u32SnsId, SAMPLE_SENSOR_INFO_S *pstSnsInfo)
{
	...
	return CVI_SENSOR_SetSnsRxAttr(u32SnsId, &stRxInitAttr);
}
...
CVI_S32 SAMPLE_COMM_ISP_SetSensorMode(SAMPLE_VI_CONFIG_S *pstViConfig, CVI_S32 pstSnsNum)
{
	...
	for (i = 0; i < pstSnsNum; i++) {
		...
		s32Ret = CVI_SENSOR_SetSnsImgMode(ViPipe, &stSnsrMode);
		if (s32Ret != CVI_SUCCESS) {
			CVI_TRACE_LOG(CVI_DBG_ERR, "sensor set image mode failed!\n");
			return s32Ret;
		}

		s32Ret = CVI_SENSOR_SetSnsWdrMode(ViPipe, wdrMode);
		if (s32Ret != CVI_SUCCESS) {
...
static SNS_BDG_MUX_MODE_E SAMPLE_COMM_ISP_GetSnsBdgMode(SNS_TYPE_E enSnsType)
{
...
{
	CVI_S32 s32Ret = -1;
	SNS_TYPE_E enSnsType = g_enSnsType[u32SnsId];
	...
	s32Ret = CVI_SENSOR_SetSnsIspAttr(IspDev, pstInitAttr);
	if (s32Ret < 0) {
	...
	if (SNSBUS_VLD(s32BusId)) {
		s32Ret = CVI_SENSOR_SetSnsI2c(IspDev, s32BusId, s32I2cAddr);
		if (s32Ret < 0) {
	...
	s32Ret = CVI_SENSOR_RegCallback(IspDev, IspDev);
	if (s32Ret != CVI_SUCCESS) {
...
CVI_S32 SAMPLE_COMM_ISP_Sensor_UnRegiter_callback(ISP_DEV IspDev)
{
	...
	s32Ret = CVI_SENSOR_UnRegCallback(u32SnsId, IspDev);
	if (s32Ret != CVI_SUCCESS) {
