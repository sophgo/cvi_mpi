				    ...
				    PAYLOAD_TYPE_E enType)
{
	...
	}else if(pAencAttrs->enType == PT_SBC){
		AENC_ATTR_SBC_S *pstAencSbc = (AENC_ATTR_SBC_S *)malloc(sizeof(
							AENC_ATTR_SBC_S));
		pstAencSbc->subbands = 8;/*4 or 8*/
		pstAencSbc->bitpool = 32;
		pstAencSbc->joint = 0;
		pstAencSbc->dualchannel = 0;
		pstAencSbc->snr = 0;
		pstAencSbc->blocks = 16;/*4, 8, 12 or 16*/
		pAencAttrs->pValue = (CVI_VOID *)pstAencSbc;
...
int printf_parse_usage(void)
{
	...
	printf("-C [codec 0:g726 1:g711A 2:g711Mu 3: adpcm 4.AAC 5.sbc]\n");
...
CVI_S32 _updata_Adec_setting(ADEC_CHN_ATTR_S *pAdecAttr)
{
	...
	}else if(pAdecAttr->enType == PT_SBC){
		ADEC_ATTR_SBC_S *pstAdecSbc = (ADEC_ATTR_SBC_S*)malloc(sizeof(
							ADEC_ATTR_SBC_S));
		pstAdecSbc->dec = 1;
		pAdecAttr->pValue = (CVI_VOID *)pstAdecSbc;
	}
...
//STEP 2:start ai and aenc
	...
	if ((enType == PT_SBC ) && (Chnsample_rate == 8000)) {
		Chnsample_rate = 16000;
		printf("SBC not support sr 8000,change to 16000.\n");
	}
...
CVI_S32 SAMPLE_AUDIO_DEBUG_SET_VOLUME(ST_AudioUnitTestCfg *testCfg)
{
	...
	printf("enter card[%d] vol[%d]\n", idevid, volumedb);
	if (err == EOF)
		printf("[Error][%s][%d]\n", __func__, __LINE__);
	printf("Set [Ain]Volume:1  Set [Aout]Volume:0 ? [0 or 1]\n");
...
CVI_S32 SAMPLE_COMM_AUDIO_CfgAcodec_Test(void)
{
	...
	printf("[%s][%d] dual_os_fast_boot not supports\n", __func__, __LINE__);
	return CVI_SUCCESS;
#endif
	/*** INNER AUDIO CODEC ***/
