...
RETRY_GET_FRAME:
			...
			CVI_VDEC_ReleaseFrame(pstVdecChn->VdecChn, &stVdecFrame);
			if (s32Ret != CVI_SUCCESS) {
				CVI_TRACE_LOG(CVI_DBG_ERR, "CVI_VPSS_GetChnFrame faile, grp:%d\n", VpssGrp);
				continue;
			}
    ...
    ************************************************/
	...
	for (int i = 0; i < stVdecCfg.s32ChnNum; i++) {
		...
		astSampleVdec[i].u32FrameBufCnt =
			(astSampleVdec[i].enType == PT_JPEG || astSampleVdec[i].enType == PT_MJPEG) ? 1 : 3;

		if (astSampleVdec[i].enType == PT_JPEG || astSampleVdec[i].enType == PT_MJPEG) {
