...
int count_repeat_pixel(CVI_U8 *src, int pixel_sz, int pixel_num)
{
	int num = pixel_num - 1;

	if (pixel_sz == 2) {
		CVI_U16 ref = ((CVI_U16 *)src)[0];
		CVI_U16 *cur_ptr = &((CVI_U16 *)src)[1];
		...
		CVI_U32 ref = ((CVI_U32 *)src)[0];
		CVI_U32 *cur_ptr = &((CVI_U32 *)src)[1];
...
void recycle_draw_obj(DRAW_OBJ *obj_vec, CVI_U32 *obj_num, int y)
{
	for (CVI_U32 i = 0; i < *obj_num; ++i) {
...
int obj_project_on_line(DRAW_OBJ *obj_vec, CVI_U32 obj_num, dlist_t *slc_list_head,
			CVI_U32 *slc_num, int y, int *next_y, int canvas_width)
{
	SLICE_LIST *slc, *slc1;
	DRAW_OBJ *obj_attr;
...
void draw_cmpr_init(Cmpr_Canvas_Ctrl *ctrl, CVI_U8 *obuf, int buf_size,
			Canvas_Attr *canvas)
{
...
void draw_cmpr_pixel(CVI_U8 *color_code_ptr, CVI_U16 length, bool is_first_pel,
			   ...
			   int y, int pixel_sz, dlist_t *seg_list_head)
{
	...
	dlist_for_each_entry_safe(seg_list_head, temp, seg, SEGMENT_LIST, item) {
		...
		if (segment->is_const) {
			if (segment->width > 0) {
				draw_cmpr_pixel((CVI_U8 *)&segment->color.code,
					segment->width, y == 0 && i == 0, ctrl);
			}
		} else {
			int rep_cnt;
			CVI_U8 *src_ptr = segment->color.buf;

			if (!segment->is_cmpr) {
				obj_vec = obj_vec;
				for (int pel_i = 0; pel_i < segment->width; pel_i += rep_cnt) {
					CVI_U8 *cur_ptr = &src_ptr[pel_i * pixel_sz];
...
void plot_segments_on_line(DRAW_OBJ *obj_vec, CVI_U32 obj_num,
				int width, int y, int pixel_sz, int *next_y,
				dlist_t *seg_list_head, CVI_U32 bg_color_code)
{
	...
	if (obj_project_on_line(obj_vec, obj_num, &slc_list_head, &slc_num, y, next_y, width) == 0) {
		...
		bg_seg->segment.width = (CVI_U16)width;
		...
		if (obj_idx >= 0) {
			...
			if (seg->segment.is_const) {
				...
				if (!seg->segment.is_cmpr) {
					...
					seg->segment.bs_len = &((CVI_U16 *)obj->color.buf)[incr_y];
			 ...
			 CVI_U8 *obuf, int buf_size, CVI_U32 *p_osize)
{
	...
	if (obj_num > 0) {
		for (CVI_U32 var = 0; var < obj_num; ++var) {
	...
	CVI_U32 stream_sz = (ctrl.bitstream.bit_pos + 7) >> 3; // in byte
...
{
	if (pixel_sz == 4) {
		fill_n((CVI_U32 *)dest, width, code);
	} else if (pixel_sz == 2) {
		fill_n((CVI_U16 *)dest, width, (CVI_U16)code);
	} else {
		fill_n((CVI_U8 *)dest, width, (CVI_U8)code);
	}
}
	...
	for (int obj_i = 0; obj_i < obj_vec.size(); obj_i++) {
		...
		if (obj_attr.type == STROKE_RECT) {
			...
			for (int y = obj_attr.rect.y;
				 ...
				 y++) {
				CVI_U8 *dest_ptr = &obuf[(y * canvas->width +
			...
			for (int y = y2;
				 ...
				 y++) {
				CVI_U8 *dest_ptr = &obuf[(y * canvas->width + obj_attr.rect.x) * pixel_sz];
			...
			CVI_U8 *src_ptr = obj_attr.color.buf;
...
void draw_canvas_raw_line(CVI_U8 *obuf, Canvas_Attr *canvas,
			  DRAW_OBJ *obj_vec, int y, int pixel_sz,
			 SEGMENT *segment_vec)
{
	CVI_U8 *dest_ptr = obuf + (y * canvas->width * pixel_sz);
		...
		int height, int buf_size, CVI_U32 *p_osize)
{
	size_t pixel_sz = osd_cmpr_get_pixel_sz(canvas->format);
	CVI_U8 *cur_ptr = ibuf;
	CVI_U16 *meta_ptr = (CVI_U16 *)obuf;
	size_t meta_sz = height << 1;
	CVI_U8 *bs_ptr = &obuf[meta_sz];
...
void set_rect_obj_attr(DRAW_OBJ *obj_attr, Canvas_Attr *canvas,
			   CVI_U32 color_code, int pt_x, int pt_y, int width,
...
void set_line_obj_attr(DRAW_OBJ *obj_attr, Canvas_Attr *canvas,
			   CVI_U32 color_code, int pt_x0, int pt_y0, int pt_x1,
...
void OSDC_set_rect_obj_attr(Canvas_Attr *canvas, DRAW_OBJ *obj, CVI_U32 color_code, int pt_x, int pt_y, int width,
				int height, bool is_filled, int thickness)
{
...
#if (CMPR_CANVAS_DBG)
int OSDC_draw_canvas_raw_buffer(Canvas_Attr *canvas, DRAW_OBJ *obj_vec, CVI_U8 *obuf)
int OSDC_draw_canvas_raw_buffer2(Canvas_Attr *canvas, DRAW_OBJ *obj_vec, CVI_U8 *obuf);
#endif
