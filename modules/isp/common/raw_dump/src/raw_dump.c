...
static CVI_S32 calculate_blk_cnt(CVI_U32 u32BlkSize)
{
	...
	LOGOUT("ion info, total_mem: %d, alloc_mem: %d, blkSize: %d\n",
		total_mem, alloc_mem, u32BlkSize);
...
#error "should check here when porting new chip"
	...
	stDumpInfo.phy_size = sizeof(*pFullVbPhyAddrList) * cfg.u32BlkCnt;
	...
	RETURN_FAILURE_IF(tempVbBlkCnt == 0);
