...
static int iso_lut[ISP_AUTO_ISO_STRENGTH_NUM] = {
...
static int get_rgbmap_block_wBit(int iso);
...
static void show_ion_debug_info(void)
{
	CVI_S32 total_mem;
	CVI_S32 alloc_mem;
	CVI_ISP_GetIonInfo(&total_mem, &alloc_mem, NULL, NULL);

	LOGOUT("ION total mem:%d\n", total_mem);
	LOGOUT("ION alloc mem:%d\n", alloc_mem);
	LOGOUT("raw replay frame number = (ION total mem - ION alloc mem) / rawFrameSize\n\n");
}

CVI_S32 set_raw_replay_data(VI_PIPE ViPipe, const CVI_VOID *header, const CVI_VOID *data,
							CVI_U32 totalFrame, CVI_U32 curFrame, CVI_U32 rawFrameSize)
{
	...
	if (curFrame == 0) {
		...
		if (pRawReplayCtx->PoolID == VB_INVALID_POOLID) {
			...
			LOGOUT("create vb pool cnt: %d, blksize: %d phyAddr: %lld\n", 1,
...
static CVI_S32 get_rgbmap_buf(RAW_REPLAY_CTX_S *pCtx)
{
	...
	s32Ret = CVI_VI_GetRgbMapLeBuf(pCtx->ViPipe, &pCtx->drvInfo[0]);
	...
	if (pCtx->pRawHeader[0].enWDR) {
		s32Ret = CVI_VI_GetRgbMapSeBuf(pCtx->ViPipe, &pCtx->drvInfo[1]);
		RETURN_FAILURE_IF(s32Ret != CVI_SUCCESS);
...
static int get_rgbmap_block_wBit(int iso)
{
	...
	if (stTNRAttr.enOpType == OP_TYPE_MANUAL) {
		...
		int wBit_l = stTNRAttr.stAuto.MtDetectUnit[lower_idx];
		int wBit_u = stTNRAttr.stAuto.MtDetectUnit[upper_idx];
		int iso_l = iso_lut[lower_idx];
		int iso_u = iso_lut[upper_idx];
