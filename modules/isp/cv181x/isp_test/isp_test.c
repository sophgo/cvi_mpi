...
#define RANDOM_INIT_ATTR(attr, reg, min, max) do {\
	...
	ISP_DEBUG(LOG_INFO, "random init: %s(%s) [%d-%d] = %d (%#08x)\n", #attr, #reg, min, max, attr, (CVI_U32)attr);\
} while (0)
...
#define PRINT_ATTR(attr, reg, min, max) ISP_DEBUG(LOG_INFO, "%s(%s) [%d-%d] = %d\n", #attr, #reg, min, max, attr)

#define PRINT_TEST_RESULT(compareResult, block) do {\
	if (compareResult) \
		ISP_DEBUG(LOG_ERR, "%s FAIL\n", #block);\
	else \
		ISP_DEBUG(LOG_INFO, "%s SUCCESS\n", #block);\
} while (0)
...
int isp_block_mpi_setest(VI_PIPE ViPipe, int blockNum, int method)
{
	...
	case ISP_TEST_PQ_BIN: {
		...
		// Demosaic Enable should be 1
		{
			...
			if (attr.Enable == 0) {
				ISP_DEBUG(LOG_ERR, ".Enable should be 1\n");
			} else {
				ISP_DEBUG(LOG_DEBUG, ".Enable is 1\n");
			}
		}
		...
		// CVI_PQ_BIN_GenerateBinFile
		{
			...
			if (ret != CVI_SUCCESS) {
				...
				ISP_DEBUG(LOG_ALERT, "CVI_BIN_ImportBinData error! errno(0x%x)\n", ret);
				break;
			}
			u32TempLen = fwrite(pBuffer, 1, u32BinLen, fd);
			if (u32TempLen != u32BinLen) {
				ISP_DEBUG(LOG_ALERT, "write bin buffer to %s fail\n", TEST_BIN);
				free(pBuffer);
				fclose(fd);
				break;
			}
		...
		// CVI_PQ_BIN_GetBinExtraAttr
		{
			...
			if (strcmp((CVI_CHAR *)BinExtraAttr.Author, "kidd")) {
				ISP_DEBUG(LOG_ERR, ".Author should be kidd, but get %s\n", BinExtraAttr.Author);
			}

			if (strcmp((CVI_CHAR *)BinExtraAttr.Desc, "test-default")) {
				ISP_DEBUG(LOG_ERR, ".Desc should be test-default, but get %s\n", BinExtraAttr.Desc);
			}

			if (strcmp((CVI_CHAR *)BinExtraAttr.Time, "2020-04-10")) {
				ISP_DEBUG(LOG_ERR, ".Time should be 2020-04-10, but get %s\n", BinExtraAttr.Time);
			}
		}
	...
	// Create folder recursively
	{
		...
		for (int i = 0 ; i < subFolderCnt-1 ; i++) {
			if (stat(subFolder[i], &status) == -1) {
				ISP_DEBUG(LOG_DEBUG, "Folder path not exist! (%s)\n", subFolder[i]);
...
static CVI_S32 isp_test_yuv_get(CVI_U8 chn, CVI_U8 **yuvBuf, CVI_U32 *yuvSize)
{
	...
	if (CVI_VI_GetChnFrame(0, chn, &stVideoFrame, 1000) == 0) {
		...
		if (yuvBuf == CVI_NULL) {
			ISP_DEBUG(LOG_WARNING, "yuvBuf pointer is NULL\n");
			return CVI_FAILURE;
		}
		*yuvBuf = tmpAddr = calloc(1, image_size);
		if (*yuvBuf == CVI_NULL) {
			ISP_DEBUG(LOG_WARNING, "yuvBuf allocate fail. Size =  %zu\n", image_size);
			return CVI_FAILURE;
		}
		ISP_DEBUG(LOG_WARNING, "width: %d, height: %d, total_buf_length: %zu\n",
...
#if 0
			ISP_DEBUG(LOG_WARNING, "plane(%d): paddr(%lu) vaddr(%p) stride(%d) length(%d)\n",
