...
static CVI_S32 set_sensorname_to_environment(VI_PIPE ViPipe)
{
	...
	if (ret != 0) {
		ISP_DEBUG(LOG_WARNING, "set env value:%s fail.\n", chEnvValue);
	}
...
CVI_S32 CVI_ISP_SnsInit(VI_PIPE ViPipe, ISP_SNS_CFG_S *pstSnsCfg)
{
	...
	CVI_U32 u32ModFd = MODFD(CVI_ID_ISP, ViPipe, 0);
	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_SENSOR_INIT, (CVI_VOID *)pstSnsCfg,
			sizeof(ISP_SNS_CFG_S), CVI_NULL);
	if (s32Ret != CVI_SUCCESS) {
		ISP_LOG_ERR("init sensor fail, ViPipe:%d,s32Ret:%x\n", ViPipe, s32Ret);
		return s32Ret;
	}
...
CVI_S32 CVI_ISP_SnsExit(VI_PIPE ViPipe, ISP_SNS_CFG_S *pstSnsCfg)
{
	...
	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_SENSOR_EXIT, (CVI_VOID *)pstSnsCfg,
			sizeof(ISP_SNS_CFG_S), CVI_NULL);
	if (s32Ret != CVI_SUCCESS) {
		ISP_LOG_ERR("exit sensot fail, ViPipe:%d,s32Ret:%x\n", ViPipe, s32Ret);
		return s32Ret;
	}
...
CVI_S32 CVI_ISP_Init(VI_PIPE ViPipe)
{
	...
	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_INIT, &u64PhyAddr, sizeof(CVI_U64), NULL);
...
static CVI_S32 handle_fifo_command(const char *command)
{
	...
	size_t isp_log_location_len = strlen(SET_ISP_LOG_LOCATION_CMD);
	...
	} else if (strcmp(TRIGGER_ISP_LOG_CMD, command) == 0) {
		TISP_LOG_CTRL_OPTIONS tLogOpts;

		tLogOpts.eCommand = ISP_LOG_EXPORT_LOG_TO_FILE;
	...
	} else if (strncmp(SET_ISP_LOG_LOCATION_CMD, command, isp_log_location_len) == 0) {
		TISP_LOG_CTRL_OPTIONS tLogOpts;

		tLogOpts.eCommand = ISP_LOG_SET_LOG_PATH;
		tLogOpts.pszPathName = (char *)command + isp_log_location_len;
...
CVI_S32 CVI_ISP_Exit(VI_PIPE ViPipe)
{
	...
	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_EXIT, NULL, 0, NULL);
...
CVI_S32 CVI_ISP_SetFMWState(VI_PIPE ViPipe, const ISP_FMW_STATE_E enState)
{
	...
	stPrivData.as32PrivData[0] = enState;

	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_SET_FMW_STATE, CVI_NULL,
		0, &stPrivData);
...
CVI_S32 CVI_ISP_GetFMWState(VI_PIPE ViPipe, ISP_FMW_STATE_E *penState)
{
	...
	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_GET_FMW_STATE, CVI_NULL,
		0, CVI_NULL);

	*penState = (ISP_FMW_STATE_E) s32Ret;
	...
	for (i = 0; i < AAA_TYPE_MAX; i++) {
		...
		if ((bindInfo[i].s32Id != -1) && (strlen(bindInfo[i].acLibName) != 0)) {
			...
			if (ret != ISP_3ALIB_FIND_FAIL) {
				...
				ISP_LOG_INFO("Find type %u Lib fail. Bind Lib NG %d\n", i, ret);
			}
		}
	}
...
{
	ISP_LOG_ASSERT(ViPipe >= 0 && ViPipe < VI_MAX_PIPE_NUM);

	CVI_S32 s32Ret = CVI_SUCCESS;
	MSG_PRIV_DATA_S stPrivData;
	CVI_U32 u32ModFd = MODFD2(CVI_ID_ISP, ViPipe, 0, 1);

	stPrivData.as32PrivData[0] = (CVI_S32) enIspVDType;
	stPrivData.as32PrivData[1] = u32MilliSec;

	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_GET_VD_TIMEOUT, NULL, 0, &stPrivData);
...
CVI_S32 CVI_ISP_SetRegister(VI_PIPE ViPipe, CVI_U32 u32Addr, CVI_U32 u32Value)
{
	...
	if ((u32Addr < ISP_BASE_ADDR) || (u32Addr > (ISP_BASE_ADDR + ISP_REG_RANGE))) {
		ISP_LOG_INFO("addr %#x is over ISP register range\n", u32Addr);
		return CVI_FAILURE;
	}
	//s32Ret = regInit();
	if (s32Ret != CVI_SUCCESS) {
		ISP_LOG_INFO("Init register address fail\n");
		return CVI_FAILURE;
	}
...
CVI_S32 CVI_ISP_QueryInnerStateInfo(VI_PIPE ViPipe, ISP_INNER_STATE_INFO_S *pstInnerStateInfo)
{
	...
	if (pstIspCtx->u8SnsWDRMode == WDR_MODE_2To1_LINE) {
		...
		if (pstIspCtx->u8SnsWDRMode != WDR_MODE_NONE) {
			ISP_LOG_INFO("This Wdr mode not support get exposure ratio now\n");
		}
	}
	return CVI_SUCCESS;
}
...
{
	ISP_LOG_ASSERT(ViPipe >= 0 && ViPipe < VI_MAX_PIPE_NUM);
	ISP_LOG_ASSERT(pstSmartInfo != CVI_NULL);

	CVI_S32 s32Ret = CVI_SUCCESS;
	MSG_PRIV_DATA_S stPrivData;
	CVI_U32 u32ModFd = MODFD(CVI_ID_ISP, ViPipe, 0);

	stPrivData.as32PrivData[0] = TimeOut;

	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_SET_SMART_INFO, (CVI_VOID *)pstSmartInfo,
		sizeof(ISP_SMART_INFO_S), &stPrivData);
...
CVI_S32 CVI_ISP_GetSmartInfo(VI_PIPE ViPipe, ISP_SMART_INFO_S *pstSmartInfo)
{
	...
	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_GET_SMART_INFO, pstSmartInfo,
		0, CVI_NULL);
...
CVI_S32 CVI_ISP_MediaVideoInit(CVI_BOOL IsRawReplayMode)
{
	CVI_S32 s32Ret = CVI_SUCCESS;
	MSG_PRIV_DATA_S stPrivData;
	CVI_U32 u32ModFd = MODFD(CVI_ID_ISP, 0, 0);
	stPrivData.as32PrivData[0] = IsRawReplayMode;

	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_MEDIA_VIDEO_INIT, CVI_NULL, 0, &stPrivData);
...
CVI_S32 CVI_ISP_MediaVideoDeinit(CVI_BOOL IsRawReplayMode)
{
	...
	CVI_U32 u32ModFd = MODFD(CVI_ID_ISP, 0, 0);
	stPrivData.as32PrivData[0] = IsRawReplayMode;
	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_MEDIA_VIDEO_DEINIT, CVI_NULL, 0, &stPrivData);
...
CVI_S32 CVI_ISP_SetBypassFrm(VI_PIPE ViPipe, CVI_U8 bypassNum)
{
	...
	stPrivData.as32PrivData[0] = bypassNum;

	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_SET_BYPASS_FRM, CVI_NULL,
		0, &stPrivData);
...
CVI_S32 CVI_ISP_GetBypassFrm(VI_PIPE ViPipe, CVI_U8 *pbypassNum)
{
	...
	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_GET_BYPASS_FRM, NULL, 0, &stPrivData);

	if (s32Ret != CVI_SUCCESS) {
		ISP_LOG_ERR("%d\n", s32Ret);
	}

	*pbypassNum = stPrivData.as32PrivData[0];
...
CVI_S32 CVI_ISP_GetIonInfo(CVI_S32 *ptotal, CVI_S32 *pused, CVI_S32 *pmfree, CVI_S32 *ppeak)
{
	...
	s32Ret = CVI_MSG_SendSync(u32ModFd, MSG_CMD_ISP_GET_ION_INFO, &dummy, sizeof(CVI_S32), &stPrivData);

	if (s32Ret != CVI_SUCCESS) {
		ISP_LOG_ERR("%d\n", s32Ret);
	}
	if (ptotal) {
		*ptotal = stPrivData.as32PrivData[0];
	}
	if (pused) {
		*pused = stPrivData.as32PrivData[1];
	}
	if (pmfree) {
		*pmfree = stPrivData.as32PrivData[2];
	}
	if (ppeak) {
		*ppeak = stPrivData.as32PrivData[3];
	}
	return s32Ret;
}
