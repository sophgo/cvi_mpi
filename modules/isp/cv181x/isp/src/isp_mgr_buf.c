...
//	printf("size,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%ld\n",
...
//
//	printf("offset,%ld,%ld,%ld,%ld,%ld,%ld,%ld\n",
//		offsetof(struct isp_shared_buffer, mlsc),
//		offsetof(struct isp_shared_buffer, drc),
//		offsetof(struct isp_shared_buffer, tnr),
//		offsetof(struct isp_shared_buffer, blc),
//		offsetof(struct isp_shared_buffer, csc),
//		offsetof(struct isp_shared_buffer, wb),
//		offsetof(struct isp_shared_buffer, ispCtx));
	...
	printf("ISP Vipipe(%d) mmap pa(%#llx) va(0x%p) size(%d)\n",
	...
	if (ev->frame_idx != offsetof(struct isp_shared_buffer, ispCtx)) {
		ISP_LOG_ERR("isp header files do not match!\n");
		ISP_LOG_ASSERT(0);
	}
...
CVI_S32 isp_mgr_buf_uninit(VI_PIPE ViPipe)
{
	...
	if (CVI_SYS_Munmap(runtime->vaddr, runtime->len) < 0) {
