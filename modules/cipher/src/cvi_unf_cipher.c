					  ...
					  CVI_U32 u32ByteLength, CVI_BOOL bOddKey)
{
	...
	if (cp->ctrl.enAlg == CVI_UNF_CIPHER_ALG_DMA) {
		...
		/* Tag should be set here, after EVP_DecryptInit_ex but before EVP_DecryptUpdate */
		/* This will be done by CVI_UNF_CIPHER_SetTag if called */
...
CVI_S32 CVI_UNF_CIPHER_SetTag(CVI_CIPHER_HANDLE hCipher, const CVI_U8 *pu8Tag, CVI_U32 u32TagLen)
{
	CVI_CIPHER_CTX *cp;
	...
	if (u32TagLen != cp->u32TagLen) {
		_cc_error("u32TagLen %u != %u\n", u32TagLen, cp->u32TagLen);
		return CVI_FAILURE;
	}

	/* Set tag */
	EVP_CIPHER_CTX_ctrl(cp->cp_ctx, EVP_CTRL_AEAD_SET_TAG, u32TagLen, (void *)pu8Tag);
