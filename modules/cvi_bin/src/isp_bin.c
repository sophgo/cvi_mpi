...
CVI_S32 header_bin_GetHeaderDescInfo(CVI_CHAR *pOutDesc, enum CVI_BIN_CREATMODE inCreatMode)
{
	...
	pOutDesc[sizeof(binGerrit) - 1] = '\0';
	strcpy(pOutDesc, binGerrit);
...
static CVI_S32 isp_index_bin_getparamfrombinImp(enum CVI_BIN_SECTION_ID id, CVI_U8 *addr, CVI_U32 binSize, CVI_U32 indexOffset)
{
	...
	CVI_TRACE_ISP_BIN(CVI_DBG_DEBUG, "apply bin setting to %d by index\n", ViPipe);
...
static CVI_S32 isp_getIndexBinParam(enum CVI_BIN_SECTION_ID id, CVI_U8 *buf, ISP_Parameter_Structures *pst, CVI_U32 indexOffset)
{
	...
	if ((id < CVI_BIN_ID_MIN) || (id >= CVI_BIN_ID_MAX)) {
		CVI_TRACE_ISP_BIN(CVI_DBG_ERR, "module id %d value error\n", id);
		return CVI_FAILURE;
	}
...
static CVI_S32 isp_bin_checkMd5(VI_PIPE ViPipe, CVI_U8 *addr, CVI_U32 binSize)
{
	...
	if (strcmp(binMd5, curMd5)) {
		...
		if (size != binSize) {
			CVI_TRACE_ISP_BIN(CVI_DBG_ERR, "vi(%d), Bin Size not matched(%u vs %u),", ViPipe, size, binSize);
			CVI_TRACE_ISP_BIN(CVI_DBG_ERR, "%s%s",
...
static CVI_S32 isp_set_paramstruct(VI_PIPE ViPipe, ISP_Parameter_Structures *pstParaBuf)
{
	...
	// Raw-top
	CVI_ISP_SetFSHDRAttr(ViPipe, &pstParaBuf->fshdr);
	CVI_ISP_SetDPDynamicAttr(ViPipe, &pstParaBuf->dpc_dynamic);
	CVI_ISP_SetCrosstalkAttr(ViPipe, &pstParaBuf->crosstalk);
	CVI_ISP_SetBNRAttr(ViPipe, &pstParaBuf->bnr);
	CVI_ISP_SetBNRFilterAttr(ViPipe, &pstParaBuf->bnr_filter);
	CVI_ISP_SetRadialShadingAttr(ViPipe, &pstParaBuf->rlsc);
	CVI_ISP_SetRadialShadingGainLutAttr(ViPipe, &pstParaBuf->rlscLUT);
	...
	// RGB-top
	CVI_ISP_SetPFRAttr(ViPipe, &pstParaBuf->pfr);
	...
	CVI_ISP_SetDciAutoGammaAttr(ViPipe, &pstParaBuf->dciAutoGamma);
	CVI_ISP_SetPreSharpenAttr(ViPipe, &pstParaBuf->presharpen);
	CVI_ISP_SetPreSharpenRefineAttr(ViPipe, &pstParaBuf->presharpen_refine);
	CVI_ISP_SetPreSharpenEdgeExtAttr(ViPipe, &pstParaBuf->presharpen_edge_ext);
	CVI_ISP_SetTNRAttr(ViPipe, &pstParaBuf->tnr);
	CVI_ISP_SetTNRMvAttr(ViPipe, &pstParaBuf->tnr_mv);
	CVI_ISP_SetTNRPsAttr(ViPipe, &pstParaBuf->tnr_ps);
	CVI_ISP_SetTNRNrAttr(ViPipe, &pstParaBuf->tnr_nr);
	CVI_ISP_SetCNRAttr(ViPipe, &pstParaBuf->cnr);
	CVI_ISP_SetCNRFilterAttr(ViPipe, &pstParaBuf->cnr_filter);
...
static CVI_S32 isp_get_paramstruct(VI_PIPE ViPipe, ISP_Parameter_Structures *pstParaBuf)
{
	...
	// Raw-top
	CVI_ISP_GetFSHDRAttr(ViPipe, &pstParaBuf->fshdr);
	CVI_ISP_GetDPDynamicAttr(ViPipe, &pstParaBuf->dpc_dynamic);
	CVI_ISP_GetCrosstalkAttr(ViPipe, &pstParaBuf->crosstalk);
	CVI_ISP_GetBNRAttr(ViPipe, &pstParaBuf->bnr);
	CVI_ISP_GetBNRFilterAttr(ViPipe, &pstParaBuf->bnr_filter);
	CVI_ISP_GetRadialShadingAttr(ViPipe, &pstParaBuf->rlsc);
	CVI_ISP_GetRadialShadingGainLutAttr(ViPipe, &pstParaBuf->rlscLUT);
	...
	// RGB-top
	CVI_ISP_GetPFRAttr(ViPipe, &pstParaBuf->pfr);
	...
	CVI_ISP_GetDciAutoGammaAttr(ViPipe, &pstParaBuf->dciAutoGamma);
	CVI_ISP_GetPreSharpenAttr(ViPipe, &pstParaBuf->presharpen);
	CVI_ISP_GetPreSharpenRefineAttr(ViPipe, &pstParaBuf->presharpen_refine);
	CVI_ISP_GetPreSharpenEdgeExtAttr(ViPipe, &pstParaBuf->presharpen_edge_ext);
	CVI_ISP_GetTNRAttr(ViPipe, &pstParaBuf->tnr);
	CVI_ISP_GetTNRMvAttr(ViPipe, &pstParaBuf->tnr_mv);
	CVI_ISP_GetTNRPsAttr(ViPipe, &pstParaBuf->tnr_ps);
	CVI_ISP_GetTNRNrAttr(ViPipe, &pstParaBuf->tnr_nr);
	CVI_ISP_GetCNRAttr(ViPipe, &pstParaBuf->cnr);
	CVI_ISP_GetCNRFilterAttr(ViPipe, &pstParaBuf->cnr_filter);
