...
#ifdef PQBIN_DEFAULT_PATH_IS_MNT_CFG
...
#else
static char userBinName[WDR_MODE_MAX][BIN_FILE_LENGTH] = {
	"/mnt/sd/cvi_sdr_bin", "/mnt/sd/cvi_sdr_bin",
	"/mnt/sd/cvi_sdr_bin", "/mnt/sd/cvi_wdr_bin",
	"/mnt/sd/cvi_wdr_bin", "/mnt/sd/cvi_wdr_bin",
	"/mnt/sd/cvi_wdr_bin", "/mnt/sd/cvi_wdr_bin",
	"/mnt/sd/cvi_wdr_bin", "/mnt/sd/cvi_wdr_bin",
	"/mnt/sd/cvi_wdr_bin", "/mnt/sd/cvi_wdr_bin" };
#endif
...
static CVI_S32 check_is_register_id(enum CVI_BIN_SECTION_ID id)
{
	...
	if (id_is_valid_IspID(id) == CVI_SUCCESS) {
		...
		CVI_VI_GetPipeAttr(id - CVI_BIN_ID_ISP0, &stPipeAttr);
		if (stPipeAttr.bYuvBypassPath == CVI_TRUE) {
			return CVI_FAILURE;
		}
	}
...
CVI_S32 CVI_BIN_ImportBinData(CVI_U8 *pu8Buffer, CVI_U32 u32DataLength)
{
	...
	CVI_BOOL isVpssLoaded = CVI_FALSE;
	...
	for (CVI_U32 idx = CVI_BIN_ID_MIN; idx < CVI_BIN_ID_MAX; idx++) {
		if ((getParamFromBinFunc[idx] != NULL) && (check_is_register_id(idx) == CVI_SUCCESS)) {
			...
			if (bBlkSizeInvalid) {
				CVI_TRACE_SYS(CVI_DBG_WARN, "Size of block(%d) is 0, not import for it!\n", idx);
			} else {
				...
				if (ret != CVI_SUCCESS) {
					...
					#else
					CVI_TRACE_SYS(CVI_DBG_WARN, "Get cur block(%d) err=0x%08x & json inexistence!\n", idx, ret);
					tmpRet = CVI_BIN_JSON_ERR;
					#endif
				}
				if (idx == CVI_BIN_ID_VPSS && ret == CVI_SUCCESS) {
					isVpssLoaded = CVI_TRUE;
				}
			}
		}
	}
