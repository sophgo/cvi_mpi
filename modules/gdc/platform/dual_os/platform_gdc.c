...
static CVI_S32 gdc_comm_cfg_frame(SIZE_S *stSize, PIXEL_FORMAT_E enPixelFormat, VIDEO_FRAME_INFO_S *pstVideoFrame)
{
	...
	if (blk == VB_INVALID_HANDLE) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "Can't acquire vb block\n");
		return CVI_FAILURE;
	}
...
CVI_S32 platform_gdc_suspend(void)
{
	...
	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_SUSPEND, CVI_NULL, 0, CVI_NULL);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "suspend fail, s32Ret:%x\n", s32Ret);
		return s32Ret;
	}
...
CVI_S32 platform_gdc_resume(void)
{
	...
	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_RESUME, CVI_NULL, 0, CVI_NULL);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "resume fail, s32Ret:%x\n", s32Ret);
		return s32Ret;
	}
...
CVI_S32 platform_gdc_beginjob(GDC_HANDLE *phHandle)
{
	...
	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_BEGAIN_JOB, (CVI_VOID *)phHandle, sizeof(GDC_HANDLE), CVI_NULL);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "BeginJob fail, s32Ret:%x\n", s32Ret);
		return s32Ret;
	}
...
CVI_S32 platform_gdc_setjobidentity(GDC_HANDLE hHandle, GDC_IDENTITY_ATTR_S *identity_attr)
{
	...
	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_SET_JOB_IDENTITY, (CVI_VOID *)&cfg, sizeof(cfg), CVI_NULL);
	if (s32Ret != CVI_SUCCESS) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "BeginJob fail, s32Ret:%x\n", s32Ret);
		return s32Ret;
	}
...
CVI_S32 platform_gdc_endjob(GDC_HANDLE hHandle)
{
	...
	CVI_S32 s32Ret;
	CVI_S32 fd = MODFD2(CVI_ID_GDC, 0, 0, 1);

	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_END_JOB, (CVI_VOID *)&hHandle, sizeof(GDC_HANDLE), CVI_NULL);
	if (s32Ret != CVI_SUCCESS) {
...
CVI_S32 platform_gdc_canceljob(GDC_HANDLE hHandle)
{
	...
	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_CANCEL_JOB, (CVI_VOID *)&hHandle, sizeof(GDC_HANDLE), CVI_NULL);
	if (s32Ret != CVI_SUCCESS) {
...
CVI_S32 platform_gdc_addrotationtask(GDC_HANDLE hHandle, const GDC_TASK_ATTR_S *pstTask, ROTATION_E enRotation)
{
	...
	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_ADD_ROT_TASK, (CVI_VOID *)&attr, sizeof(attr), NULL);
	if (s32Ret != CVI_SUCCESS) {
	...
	, const LDC_ATTR_S *pstLDCAttr, ROTATION_E enRotation)
{
	...
	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_ADD_LDC_TASK, (CVI_VOID *)&attr, sizeof(attr), CVI_NULL);
	if (s32Ret != CVI_SUCCESS) {
...
CVI_S32 platform_gdc_dumpmesh(MESH_DUMP_ATTR_S *pMeshDumpAttr)
{
	...
	case CVI_ID_VI:
		...
		fd = MODFD(CVI_ID_VI, ViPipe, ViChn);

		s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_VI_GET_CHN_ATTR, &stViChnAttr, sizeof(VI_CHN_ATTR_S), CVI_NULL);
		if (s32Ret != CVI_SUCCESS) {
	...
	case CVI_ID_VPSS:
		...
		fd = MODFD(CVI_ID_VPSS, VpssGrp, VpssChn);

		s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_VPSS_GET_CHN_ATTR, &stVpssChnAttr, 0, CVI_NULL);
		if (s32Ret != CVI_SUCCESS) {
...
CVI_S32 platform_gdc_loadmesh(MESH_DUMP_ATTR_S *pMeshDumpAttr, const LDC_ATTR_S *pstLDCAttr)
{
	...
	case CVI_ID_VI:
		...
		s32Ret = CVI_MSG_SendSync4(fd, MSG_CMD_VI_SET_CHN_LDC_ATTR, (CVI_VOID *)&stVILDCAttr,
	...
	case CVI_ID_VPSS:
		...
		s32Ret = CVI_MSG_SendSync4(fd, MSG_CMD_VPSS_SET_CHN_LDCATTR, (CVI_VOID *)&stVPSSLDCAttr,
...
CVI_S32 platform_gdc_getworkjob(GDC_HANDLE *phHandle)
{
	...
	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_GET_WORK_JOB, (CVI_VOID *)phHandle, sizeof(GDC_HANDLE), CVI_NULL);
	if (s32Ret) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "CVI_GDC_GetWorkJob fail,s32Ret:%x\n", s32Ret);
		return s32Ret;
	}
...
CVI_S32 platform_gdc_getchnframe(GDC_IDENTITY_ATTR_S *identity, VIDEO_FRAME_INFO_S *pstFrameInfo, CVI_S32 s32MilliSec)
{
	...
	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_GET_CHN_FRAME, (CVI_VOID *)&cfg, sizeof(cfg), CVI_NULL);
	if (s32Ret) {
...
CVI_S32 platform_gdc_attachvbpool(MMF_CHN_S *pChn, VB_POOL u32VbPool)
{
	...
	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_ATTACH_VB_POOL, (CVI_VOID *)&cfg, sizeof(cfg), CVI_NULL);
	if (s32Ret) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "gdc attach mod %d dev %d chn %d vb pool %d fail, Ret[%d]\n"
			, cfg.Chn.enModId, cfg.Chn.s32DevId, cfg.Chn.s32ChnId, cfg.VbPool, s32Ret);
		return s32Ret;
	}
...
CVI_S32 platform_gdc_detachvbpool(MMF_CHN_S *pChn)
{
	...
	s32Ret = CVI_MSG_SendSync(fd, MSG_CMD_GDC_DETACH_VB_POOL, (CVI_VOID *)&cfg, sizeof(cfg), CVI_NULL);
	if (s32Ret) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "gdc detach mod %d dev %d chn %d vb pool fail, Ret[%d]\n"
			, cfg.Chn.enModId, cfg.Chn.s32DevId, cfg.Chn.s32ChnId, s32Ret);
		return s32Ret;
	}
