...
static void *device_auto_detect(void *arg)
{
	...
	// if(stSnsAhdObj[ViPipe]->pfnAhdInit)
	// 	stSnsAhdObj[ViPipe]->pfnAhdInit(ViPipe, false);
	while (1) {
		if (stSnsAhdObj[ViPipe]->pfnDetectAhdStatus) {
			CVI_S32 ret = 0;
			ret = stSnsAhdObj[ViPipe]->pfnDetectAhdStatus(ViPipe, signal_type_old, &signal_type);
			if (ret == CVI_FAILURE) {
				continue;
			}
		} else {
			usleep(100 * 1000);
			if(stSnsAhdObj[ViPipe]->pfnGetAhdMode)
				signal_type = stSnsAhdObj[ViPipe]->pfnGetAhdMode(ViPipe);
		...
		if (User_Callback[ViPipe]) {
			CVI_TRACE_MSG(CVI_DBG_ERR, "Sensor %d callback ahd_mode: %d\n", ViPipe, signal_type);
			User_Callback[ViPipe](ViPipe, signal_type);
		}
...
CVI_S32 platform_sns_EnableDetect(VI_PIPE ViPipe)
{
	...
	if (arg == NULL) {
		CVI_TRACE_LOG(CVI_DBG_ERR, "malloc auto detect running thread arg failed\n");
		return s32Ret;
	}
	...
	*arg = ViPipe;
	...
	s32Ret = pthread_create(&g_sensorDet_thid[ViPipe], &attr, device_auto_detect, arg);
	if (s32Ret != 0) {
		SNS_DBG_PRT("create auto detect running thread failed!, error: %d, %s\r\n",
...
CVI_S32 platform_sns_GetAhdStatus(SNS_STATUS_MSG_S *pstStatus)
{
	...
	g_AhdMode[ViPipe] = pstStatus->eMode = stSnsAhdObj[ViPipe]->pfnGetAhdMode(ViPipe);
