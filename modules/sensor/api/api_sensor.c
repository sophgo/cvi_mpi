...
{
	return platform_sns_parseini(sensor_cfg);
}

CVI_S32 CVI_SNS_SetIniPath(const CVI_CHAR *iniPath)
{
	return platform_sns_setinipath(iniPath);
}
...
{
	platform_AHDRegisterDetect(ViPipe, CB);
...
{
	return platform_sns_EnableDetect(ViPipe);
}
