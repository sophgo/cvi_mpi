...
CVI_S32 platform_vb_mmappool(VB_POOL Pool)
{
	...
	if (cfg.remap_mode == VB_REMAP_MODE_CACHED)
		vaddr = platform_sys_mmapcache(cfg.mem_base, cfg.blk_cnt * cfg.blk_size);
	else
		vaddr = platform_sys_mmap(cfg.mem_base, cfg.blk_cnt * cfg.blk_size);
	if (vaddr == NULL) {
...
CVI_S32 platform_vb_munmappoold(VB_POOL Pool)
{
	...
	if (platform_sys_munmap(pstVbPool->vmemBase, pstVbPool->u32BlkCnt * pstVbPool->u32BlkSize) != CVI_SUCCESS) {
...
CVI_S32 platform_vb_getblockviraddr(VB_POOL Pool, VB_BLK Block, void **ppVirAddr)
{
	...
	poolId = platform_vb_handle2poolid(Block);
	if (poolId != Pool) {
