...
int load_meshdata(const char *path, MESH_DATA_ALL_S *pmeshdata, const char *bindName)
{
	...
	if (path == NULL) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "file null\n");
		return -1;
	}
	...
	if (count_grid > 0) {
		...
		// hw mesh
		if (fread(pmeshdata->pmesh_src, sizeof(int), (count_grid * 2 * 4), fpGrid)
				!= (size_t)(count_grid * 2 * 4)) {
			CVI_TRACE_GDC(CVI_DBG_ERR, "read file fail, %s\n", path);
			return -1;
		}
		if (fread(pmeshdata->pmesh_dst, sizeof(int), (count_grid * 2 * 4), fpGrid)
			!= (size_t)(count_grid * 2 * 4)) {
			CVI_TRACE_GDC(CVI_DBG_ERR, "read file fail, %s\n", path);
			return -1;
		}
	...
	if (pmeshdata->node_index > 0) {
		// hw node
		if (fread(pmeshdata->pnode_src, sizeof(int), (pmeshdata->node_index * 2), fpGrid)
			!= (size_t)(pmeshdata->node_index * 2)) {
			CVI_TRACE_GDC(CVI_DBG_ERR, "read file fail, %s\n", path);
			return -1;
		}
		if (fread(pmeshdata->pnode_dst, sizeof(int), (pmeshdata->node_index * 2), fpGrid)
			!= (size_t)(pmeshdata->node_index * 2)) {
			CVI_TRACE_GDC(CVI_DBG_ERR, "read file fail, %s\n", path);
			return -1;
		}
...
{
	if (!path) {
		CVI_TRACE_GDC(CVI_DBG_DEBUG, "path [(null)]\n");
		return -1;
	}
