...
static CVI_S32 _get_1st_src_midxy(int num_tilex_s1, LDC_ATTR *cfg
	, COORD2D_INT_HW *src_1st_list, int tidx, int tidy
		...
		for (int midx = 0; midx < MESH_NUM_ATILE; midx++) {
			...
			if (pstLDCAttr->stGridInfoAttr.Enable) {
				...
				for (int knotidx = 0; knotidx < 4; knotidx++) {
					...
					if (use_count == 4) {
						map_src_knot = _find_knot_map2src(cur_dst_mesh[knotidx]
							, cfg, swmesh_hit_index[knotidx], 0);
		...
		for (int i = 0; i < 9 * (mesh_horcnt * mesh_vercnt); i++) {
			...
			for (int j = 0; j < 4; j++) {
				if (ext_str_idx <= idx &&
					...
					rgn_attr[rgn_idx].SrcRgnMeshInfoExt[i].knot[j].xcor =
						*(meshdata->pmesh_src + 8 * roi_idx + meshdata->mesh_horcnt * 8 * roi_idy + 2 * j);
					rgn_attr[rgn_idx].SrcRgnMeshInfoExt[i].knot[j].ycor =
						*(meshdata->pmesh_src + 8 * roi_idx + meshdata->mesh_horcnt * 8 * roi_idy + 2 * j + 1);
				} else {
	...
	void *mesh_vir_addr, ROTATION_E rot)
{
	...
	clock_gettime(CLOCK_MONOTONIC, &start);
#endif
	ret |= _offline_get_1st_src_mesh_table(num_tiley_s1, num_tilex_s1, cfg
		, src_1st_list, mesh_horcnt, mesh_vercnt, pstLDCAttr);
#ifdef ENABLE_PROFILE
	clock_gettime(CLOCK_MONOTONIC, &end);
...
int gdc_get_tsk_mesh_by_name(TSK_MESH_ATTR_S* tskMeshAttr)
{
	...
	if (!tskMeshAttr) {
		CVI_TRACE_GDC(CVI_DBG_ERR, "tsk mesh is null\n");
		return i;
	}

	for (i = 0; i < GDC_MAX_TSK_MESH; i++) {
		if (strcmp(tskMesh[i].Name, tskMeshAttr->Name) == 0 && tskMesh[i].paddr && tskMesh[i].vaddr) {
			tskMeshAttr->paddr = tskMesh[i].paddr;
			tskMeshAttr->vaddr = tskMesh[i].vaddr;
			CVI_TRACE_GDC(CVI_DBG_DEBUG, "got remain tsk mesh[%d-%s-%#"PRIx64"]\n"
				, i, tskMesh[i].Name, tskMesh[i].paddr);
			break;
		}
		...
		const char *name, CVI_U64 *pu64PhyAddr, CVI_VOID **ppVirAddr)
{
	...
	gdc_free_cur_tsk_mesh((CVI_CHAR *)name);
